...
const SeriesDetails = ({ series, handleGoBack }) => {
  ...
  const fetchSeriesImages = async () => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/tv/${series.id}/images?api_key=8a78bbc2059ae1af9b5db720e9ee991d`
      );
      const data = await response.json();
      setSeriesImages(data.backdrops);
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.content}>
        ...
        {seriesImages.length > 0 && (
          <View>
            <Text style={styles.sectionTitle}>Imagens da série</Text>
            {!isLoading ? (
